---
title: "Using collaborative open science tools to improve engagement with the ecology of the Guana River Estuary"
...
### 1. Response rate
We received responses from 53 individuals. Out of these, .. were unfinished. For the purposes of this report, we also took the unfinished surveys into account.

.. respondents filled in the survey based on a link received via email, .. via social media, .. via a QR code available at the GTMNERR Welcome Center, and .. via a QR code available at the kiosk at the .. Dam.
...
```{r, connected, fig.width = 7, fig.height = 3, echo = FALSE, message = FALSE}
ggplot(intro %>% 
...
```{r, engage, fig.width = 4, fig.height = 3, echo = FALSE, warning = FALSE, message = FALSE}
ggplot(intro %>% 
         filter(qname == "D-2") %>% 
...
```{r, what_data, fig.width = 7, fig.height = 6, echo = FALSE, warning = FALSE, message = FALSE}
...
Under "Other", the three types of data mentioned were: historical maps, water fowl, and dam operations.

#### 2.4 Previous experience with accessing data associated with GTMNERR
```{r, accessed_data, fig.width = 4, fig.height = 3, echo = FALSE, message = FALSE}
ggplot(intro %>% 
         filter(qname == "D-4") %>% 
         group_by(q_text) %>%
         summarize(cnt = n()) %>%
...
### 3. Feedback from respondents that have not accessed data before

Respondents that had not (yet) accessed data, the survey asked broad questions on how often they would access these data, and what they would use them for.
* These responses will still be linked to the answers in 2.3 *

```{r, no_access_howoften, fig.width = 5, fig.height = 3, echo = FALSE, message = FALSE}
ggplot(no_access %>% 
  ...
  geom_col(fill = "cyan4") +
  labs(y = "", x = "Percentage of respondents", title = "If you had the opportunity to access data in the future, how often would you access or obtain these data?") +
  theme_bw()+
  theme(plot.title.position = "plot")
```

```{r, no_access_what, fig.width = 5, fig.height = 4, echo = FALSE, message = FALSE}
ggplot(no_access %>% 
...
```{r, access_what, fig.width = 6, fig.height = 5, echo = FALSE, message = FALSE}
ggplot(yes_data %>% 
       ...
       aes(y = reorder(q_text, percentage), x = percentage, fill = q_text))+
  geom_col() +
  labs(x = "Percentage of respondents", y = "", 
       title = "What Guana Estuary data do you currently use,\nor have you used in the past?") +
...
```{r, dash_how, fig.width = 5, fig.height = 2, echo = FALSE, message = FALSE}
ggplot(dashboard %>% 
       ...
       aes(y = reorder(q_text, percentage), x = percentage, fill = q_text))+
  geom_col() +
  labs(y = "", x = "Percentage of respondents", title = "How would you prefer to access data?") +
...
```{r, dash_type, fig.width = 7, fig.height = 4, echo = FALSE, message = FALSE}
ggplot(dashboard %>% 
...
```{r, dash_form, fig.width = 7, fig.height = 4, echo = FALSE, warning = FALSE, message = FALSE}
ggplot(dashboard %>% 
...
```{r, dash_delivery, fig.width = 7, fig.height = 5, echo = FALSE, warning = FALSE, message = FALSE}
ggplot(dashboard %>% 
...
#### 6.1 Age and gender
```{r age, fig.width = 12, fig.height = 4, echo = FALSE, message = FALSE}
age <- ggplot(demogr %>% 
         filter(qname == "DE-2") %>% 
         group_by(q_text) %>%
         summarize(cnt = n()) %>%
...
``` {r, distance, fig.width = 5, fig.height = 4, echo = FALSE, message = FALSE}
ggplot(demogr %>% 
         filter(qname == "DE-4") %>% 
         group_by(q_text) %>% 
         summarize(cnt = n()) %>%
...
## Next steps
The project team will still refine details in this report, and distribute/publish the final version by the end of August 2023. This final report will also include dashboard design considerations and recommendations based on the survey results.

If you have suggestions, comments or requests, please email Dr. Geraldine Klarenberg at gklarenberg@gmail.com.
