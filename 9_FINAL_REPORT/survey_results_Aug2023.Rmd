...
txt_sources <- txt_data_yes %>% 
  ...
  arrange(field_name) %>%
  select(-main, -source, -qname, -field_no, -qname_main, -q_code)

# Get the text answers linked to advantages: extract / join by ID
...
###### Advantages ######
# Join data - advantage or disadvantage will now be separate column
yes_data_adv <- yes_data3_access %>% 
  ...
  mutate(q_text = ifelse(q_text == "Download from website (If so, what website?)", paste0("Download from website: ", text_answer), q_text)) %>% # this one might be problematic as the counts are only for the combination of category download from websites + advantage.  FIX THIS BY HAND SO THE WEBSITES ARE ALL STANDARDIZED!!!
  select(-text_answer) %>% 
  left_join(txt_adv) %>% 
  ...
  count() %>%  # e.g. for wq there were 20 people who answered yes, but since they could pick 
# more than 1 adv and disadv, these don't add up to 20. Decided to keep old table but without adv/disadv
  mutate(percentage = round(n/tot_cnt*100, 1)) %>% 
  select(-ID, -tot_cnt, -n)

##### There are still a bunch of answers where website says NA (eg ID 46?? Just did not
# fill anything in? Make it "unspecified"?)
# Lidar instead of "other" also does not show up properly



###### Disadvantages ######
# In cases of "Other" - connect to text datasets
