...
``` {r otherusage_table2, echo = FALSE, warning = FALSE, message = FALSE}
...
#kable_styling(kable(txt_data_yes_YD6))
...
levels(yes_data_table$q_text)[18:20] <- c("Slightly well", "Moderately well", "Very well", "Extremely well")
...
There is also an overview of the advantages listed per acquire method *for each dataset* but since this is an extremely dense overview, it is available upon.
...
There is also an overview of the advantages listed per acquire method *for each dataset* but since this is an extremely dense overview, it is available in the Appendix.
...
disadv_full_overview <- yes_data_disadv %>% 
  group_by(field_name, tot_cnt, q_text, Disadvantage) %>% 
  count() %>%  
  mutate(percentage = round(n/tot_cnt*100, 1)) %>% 
  select(-tot_cnt, -n)
...
p <- treemap(disadv_bysource,
            index=c("Disadvantage","q_text_num"), #index=c("q_text","Disadvantage"),
            vSize="percentage",
            type="index",
            title = "",
            palette = "Set2",
