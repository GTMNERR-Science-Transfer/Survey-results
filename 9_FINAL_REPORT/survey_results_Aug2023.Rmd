...
disadv_bysource <- yes_data_disadv %>%
  ...
  group_by(q_text, Disadvantage) %>% #percentage, 
  count() %>%  
  # ungroup() %>% 
...
names(disadv_bysource)[1] <- "labels" # q_text goes inside disadvantage so that's a label
names(disadv_bysource)[2] <- "parents" # Disadvantage is one of the main blocks so it's a parent

disadv_bysource_treemap <- disadv_bysource %>% 
  select(parents, labels, n) %>% 
