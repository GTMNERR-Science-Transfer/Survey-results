...
ggplot(intro %>% 
  ...
  labs(y = "", x = paste0("Percentage of all respondents (N = ", num_resp, ")"), 
       ...
       caption  = "NOTE: Respondents could select more than 1 response") +
...
#| Considered also taking into account the options that were NOT chosen, for example by
...
- first, from the perspective of the data access method (for example, download from website, request via email, etc), where we show the percentages of (dis)advantages for each. Note that not all data access methods were selected by the same number of respondents, so some have more nuanced (dis)advantages listed if there were more respondents.
- Second, from the perspective of the (dis)advantages, where we have grouped these together, and show which dataset types they were mentioned with. 
...
yes_data_adv <- yes_data3_access %>% 
  ...
  # Turns out not everyone filled in a source if they selected this answer. Make this look nicer than NA
  ...
  # Turns out not everyone filled in a website if they selected this answer. Make this look nicer than NA
...
disadv_bysource <- yes_data_disadv %>% 
  select(-tot_cnt) %>% 
  add_count(q_text) %>% # Total number of times access method is selected
  rename(tot_cnt = n) %>% 
