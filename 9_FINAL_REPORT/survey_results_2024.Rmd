...
For respondents that had not (yet) accessed data (N = 8), the figure below summarizes their answers from section 2.3 (datasets of interest). In this figure, the datasets are ordered according to their **average ranking**, once again 1 being the dataset of most interest. For this ranking calculation, the rank was set to 0 if it was not picked at all by respondents.
...
intro %>%
  ...
  # add categories that were not chosen and assign 0
  full_join(data.frame("ID" = rep(unique(no_access$ID), 7), 
                       "q_code" = rep(c(1,2,3,4,5,6,7), length(unique(no_access$ID))))) %>% 
  mutate(order = ifelse(is.na(order), 0, order)) %>% 
  #### TO DO: make sure this works with q_text!!! BUT THEN ALSO remove it from
  # the counts... bc they weren't picked!!!
  group_by(q_code, order) %>% #q_code instead of q_text
  summarize(cnt = n()) %>%
  filter(!is.na(q_code)) %>% # Take out the NAs associated with the 0 rank
  mutate(percentage = round(cnt/num_resp_no_data*100, 1)) %>%
  group_by(q_code) %>% 
  mutate(tot_perc = sum(percentage), ave_score = sum(order*cnt)/sum(cnt)) %>%
  
  ggplot(aes(y = reorder(q_code, ave_score, decreasing = TRUE), x = percentage, fill = fct_rev(factor(order))))+
  geom_col(color = "darkgrey") +
  ...
  #scale_fill_viridis_d(option = "plasma", name = "Rank", direction = -1)+
  scale_y_discrete(labels = function(x) str_wrap(x, width = 50))+
  geom_text(aes(x = 79, y = q_code, label = round(ave_score, 2)), check_overlap = TRUE)+
  theme_bw()+
  guides(fill = guide_legend(title = "Rank",
                             reverse = TRUE))
