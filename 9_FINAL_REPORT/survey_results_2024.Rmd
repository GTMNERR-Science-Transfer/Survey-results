...
The figure below summarizes the advantages that respondents listed per data access method. We added more detail on "Other" methods of accessing data based on responses ("Other: unspecified" is the category where respondents chose "Other" but did not provide more detail).
...
``` {r advantages_fig1, fig.width = 8, fig.height = 8, fig.align = 'center', echo = FALSE, warning = FALSE, message = FALSE}
...
The next figure, a so-called treemap, shows the same information but summarized differently - based on the advantages. The size of the rectangle for each advantage indicates the percentage of times this advantages was picked by respondents. Clicking on each rectangle then shows the data access methods for this advantage was picked; also by percentage. A hover label shows the percentages as text, as well as the advantages or access methods if the text is too long to be readily readable.
